...
const SYNERGY_RULES: { kind: string; re: RegExp }[] = [
  ...
  // 마나와 공이속은 딜 시너지가 아니라 유틸이다. 골라 찍는 트라이포드라 클래스 표에
  // 답이 없고, 찍고 온 사람만 여기서 잡힌다(서머너 슈르디 소환 / 마나 회복).
  { kind: "마나", re: /파티원[^,]{0,20}?마나[^,]{0,20}?([\d.]+)%\s*증가/ },

  /*
   * 공이속은 아직 실측하지 못해 추정 문구다. 안 잡히면 클래스 표로 떨어지므로
   * 지금보다 나빠지지 않는다.
   *
   * **자버프와 갈라야 한다.** "자신의 공격속도가 증가"는 트라이포드에 흔해서 파티원을
   * 요구한다. 둘을 한 문장에 적는 경우("공격속도와 이동속도가 …")가 있는데 툴팁 하나는
   * 한 종류만 주므로 앞 규칙이 걸리고 끝난다.
   *
   * **공증보다 뒤에 둔다.** 공격력과 공격속도를 함께 올리는 툴팁이 있다면 편성에 더
   * 무거운 공증으로 읽는 편이 맞다.
   */
  { kind: "공이속", re: /파티원[^,]{0,20}?공격속도.{0,40}?([\d.]+)%\s*증가/ },
  { kind: "공이속", re: /파티원[^,]{0,20}?이동속도.{0,40}?([\d.]+)%\s*증가/ },
];
