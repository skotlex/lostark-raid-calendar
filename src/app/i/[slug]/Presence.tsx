 ...
 *
 * **Supabase 무료 플랜의 병목은 egress(월 5GB)다.** 컴퓨트 시간은 세지 않지만
 * 오가는 바이트는 센다. 넘기면 프로젝트가 402를 반환하고 그 달이 끝날 때까지
 * 사이트가 죽는다(과금은 되지 않는다. CLAUDE.md 2-4).
 *
 * 멈추는 장치가 없으면 **누구든 탭을 열어둔 내내** 몇 초마다 왕복이 나가고 그게 전부
 * egress로 쌓인다. 사람 수에 비례하지도 않는다. 벽시계라서 한 명만 아침에 켜고 밤에
 * 닫아도 하루치가 나간다.
 *
 * 편성표는 열어둔 채 자리를 비우는 것이 기본 사용 패턴이라 이 장치가 곧 한도를
 * 지키는 장치다. 손을 대는 순간 다시 돌아온다.
 *
 * Neon을 쓰던 때는 같은 장치가 **컴퓨트가 깨어 있던 시간**을 지켰다. 옮기면서 세는
 * 축이 바뀌었을 뿐 장치가 필요한 것은 그대로다.
 */
...
export function PresenceProvider({
  ...
  const send = useCallback(async () => {
    ...
    /*
     * 열어만 두고 손을 놓았으면 멈춘다. 방치된 탭이 egress를 갉아먹지 않는 유일한 길이다.
     *
