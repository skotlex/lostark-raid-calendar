...
그래서 `src/lib/arkGridCores.ts`에 이름→단계 표를 둔다. 각인마다 해/달/별 코어가
세 벌씩 있어 표가 길어진다. loawa의 `/stat/equiparkgrid?class=<직업>` 화면에
단계가 붙은 이름이 나오므로 보고 옮겨 적으면 된다(브라우저 렌더라 자동 수집 불가).
